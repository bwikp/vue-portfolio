<template>
    <div class="containerHome">
        <div class="Landing">
                <p1>Michel Zilpa</p1>
                <p2>Développeur Web</p2>
            <div>
            <p2>En Rercher d'alternance</p2>
            </div>
        </div>
        <div class="landingLink">
            <a href="https://simplon.co/formation/concepteur-developpeur-d-applications/12" target="_blank"><img class="svgIcon" src="/src/assets/SIMPLON.svg" alt=""></a>
            <a href="https://github.com/bwikp" target="_blank"><img class="svgIcon" src="/src/assets/github-mark.svg"/></a>
            <a href="https://github.com/bwikp" target="_blank"><img class="svgIcon" src="/src/assets/linkedin.svg"/></a>
        </div>
    </div>
</template>
<style scoped>
    
</style>